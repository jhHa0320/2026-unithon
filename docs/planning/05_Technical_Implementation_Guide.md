...
## Phase 6-1. 결제수단 연동·완료 Mock 화면

결제 대기 화면에서 사용자가 확인을 누른 다음 단계는 실제 PG/코레일 결제 확정 API를 호출하지 않고, 앱 내부에만 존재하는 Mock 데이터로 화면 전환만 수행한다. 코레일+ 앱의 실제 결제 화면을 accessibility로 조작해 만들지 않고, 우리 앱 자체 오버레이에서 독립적으로 렌더링한다.

| **구분** | **In-Scope (구현)** | **Out-of-Scope (미구현)** |
| --- | --- | --- |
| 결제수단 | 더미 카드 정보(예: "**** **** **** 1234")를 로컬에 미리 등록해두고 표시만 함 | 실제 카드사/PG 등록·본인인증 |
| 결제 처리 | 확인 탭 후 로컬에서 짧은 로딩 UI(1~2초)를 보여준 뒤 "결제 완료" 더미 화면으로 전환 | PG 결제 승인 API 호출, 코레일 실시간 결제 확정 API 호출 |
| 예매 확정 | "예매가 완료되었습니다" 더미 화면 표시 | 실제 좌석 확정·티켓 발급 |

구현은 Android 클라이언트의 로컬 상태 전환만으로 이루어진다(예: `PaymentMockRepository.completePayment()`가 코루틴 delay 후 성공 결과를 반환). backend의 `/api/v1/decide` 파이프라인이나 실제 네트워크 호출과는 무관하며, 카드번호 등은 처음부터 실제 값이 아닌 placeholder만 사용한다. 데모 화면에는 "Demo" 또는 "테스트" 표기를 남겨 실제 결제로 오인되지 않도록 한다.
...
결제수단 연동·완료 화면은 코레일+ 앱의 실제 결제 화면을 accessibility로 조작하지 않고 우리 앱 자체 오버레이에서만 렌더링하는 Mockup이므로, 실제 결제 승인/거래에 개입하지 않는다는 점을 정책 심사·발표에서 함께 소명한다.
...
"결제 대기 확인 화면에서 사용자가 확인을 누르면, 로컬 Mock 데이터로 결제수단 연동 화면과 결제 완료 화면을 순서대로 보여주는 UI를 구현해줘. 실제 PG/코레일 API는 호출하지 않아."
...
□ 결제수단 연동·완료 Mock 화면 (실제 PG/코레일 API 미호출 확인)
...
| 결제수단 연동·완료 Mock 화면 | 3/10 | 로컬 상태 전환만 필요, 실 PG/코레일 API 연동 없음 |
