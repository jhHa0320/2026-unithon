...
6. **목표가 달성되었으면 status를 "DONE"으로** 합니다. **history를 반드시 확인하세요** —
   ...
   단, **중간 단계의 "완료" 문구에 속지 마세요.** "예약이 완료되었습니다"는 좌석을 잡았다는
   뜻일 뿐 아직 결제가 남은 상태입니다. goal이 예매·구매·주문이라면 **결제까지 끝나야
   완료**입니다 — 화면에 "결제", "결제하기", "결제기한" 같은 요소가 보이면 DONE이 아니라
   결제 단계로 계속 진행하세요.
...
9. **코레일의 출발일 선택은 2단계입니다: 날짜를 먼저 고르고, 시간(시각)을 따로 고릅니다.**
   날짜를 선택했다고 끝이 아닙니다 — 화면에 시간 선택(시각 목록/휠)이 보이면 goal의
   출발 시각과 맞는 시간을 이어서 선택한 뒤에 조회/확인으로 넘어가세요.
